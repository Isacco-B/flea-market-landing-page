...
const languages = {
  en: { file: "en.json", flag: flags.en },
  it: { file: "it.json", flag: flags.it },
  es: { file: "es.json", flag: flags.es },
};
...
async function changeLanguage(lang) {
  ...
  langFlag.innerHTML = languages[lang].flag;
  const langData = await fetchLanguageData(lang);
  updateContent(langData);
  langMenu.classList.toggle("hidden");
  root.classList.toggle("opacity-40");
}
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  const langData = await fetchLanguageData(activeLang);
  updateContent(langData);
});
...
document.addEventListener("click", (event) => {
  if (!langMenu.contains(event.target) && !langBtn.contains(event.target)) {
    ...
    root.classList.remove("opacity-40");
  }
});
